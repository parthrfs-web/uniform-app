...
export function parseWorkbookAuto(
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    if (!match) {
      if (hasAnyContent(matrix)) {
        const fallback = parseRowsWithoutHeader(
          matrix,
          sheetName,
          sourceFile,
          seenEmployeeCodes,
    ...
    for (
      let rowIndex = match.headerRowIndex + 1;
      ...
      if (match.nameIndex !== undefined && match.unitIndex !== undefined) {
        ...
        if (
          name &&
          unitName &&
          !seenEmployeeCodes.has(normalize(employeeCode))
        ) {
          ...
          employees.push({
            ...
            name,
            fatherName:
              match.fatherIndex !== undefined
                ? text(row[match.fatherIndex]) || "Not provided"
                : "Not provided",
            unitId: slug(unitName),
            unitName,
            joiningDate:
              match.joiningIndex !== undefined
                ? formatDate(row[match.joiningIndex]) || "Not provided"
                : "Not provided",
            status:
              match.statusIndex !== undefined &&
              text(row[match.statusIndex]).toLowerCase() === "inactive"
                ? "inactive"
                : "active",
          });
          employeeCount += 1;
      ...
      if (match.itemIndex !== undefined && match.quantityIndex !== undefined) {
        ...
        const issuedAt =
          match.dateIndex !== undefined
            ? isoDate(row[match.dateIndex])
            : inferredDate;
        ...
        if (
          ...
          issues.push({
            row: rowNumber,
            message: `${sheetName}: item, quantity, and date/month are required for distribution rows.`,
          });
          continue;
        }
        distributions.push({
      ...
      for (const column of match.itemQuantityColumns) {
        ...
        if (!issuedAt) {
          issues.push({
            row: rowNumber,
            message: `${sheetName}: ${column.itemName} has quantity but no date/month could be detected.`,
          });
          continue;
        }
        distributions.push({
    ...
    if (employeeCount || distributionCount) {
      sheetSummaries.push(
        `${sheetName}: ${employeeCount} employee row(s), ${distributionCount} distribution row(s)`,
      );
    } else {
      sheetSummaries.push(
        `${sheetName}: header detected but no valid rows found`,
      );
    }
  }
  ...
  if (!employees.length && !distributions.length && !issues.length) {
    issues.push({
      row: -1,
      message:
        "No employee or distribution rows were detected. The app looks for employee code plus name/unit, or employee code plus item quantities/date.",
    });
  }
...
function detectHeader(matrix: Matrix): HeaderMatch | null {
  ...
  for (let rowIndex = 0; rowIndex < limit; rowIndex += 1) {
    const headers = (matrix[rowIndex] ?? []).map((cell) =>
      normalize(text(cell)),
    );
    ...
    const itemQuantityColumns =
      itemIndex !== undefined && quantityIndex !== undefined
        ? []
        : detectWideItemColumns(matrix, rowIndex, headers);

    const hasEmployeeShape = nameIndex !== undefined && unitIndex !== undefined;
    const hasLongDistributionShape =
      itemIndex !== undefined &&
      quantityIndex !== undefined &&
      dateIndex !== undefined;
    const hasWideDistributionShape =
      itemQuantityColumns.length > 0 &&
      (dateIndex !== undefined || inferDateFromNearbyRows(matrix, rowIndex));

    if (
      !hasEmployeeShape &&
      !hasLongDistributionShape &&
      !hasWideDistributionShape
    )
      continue;
...
function parseRowsWithoutHeader(
  ...
  const issues: ImportIssue[] = [];
  const inferredDate =
    inferDateFromSheetName(sheetName) ||
    inferDateFromNearbyRows(matrix, Math.min(5, matrix.length));

  for (let rowIndex = 0; rowIndex < matrix.length; rowIndex += 1) {
    const row = matrix[rowIndex] ?? [];
    const rowNumber = rowIndex + 1;
    const values = row
      .map((cell, index) => ({ cell, index, value: text(cell) }))
    ...
    const quantityCell = values.find(
      (item) =>
        item.value !== code &&
        !isoDate(item.cell) &&
...
function detectWideItemColumns(
  matrix: Matrix,
  headerRowIndex: number,
  headers: string[],
) {
  ...
  headers.forEach((header, index) => {
    if (
      !header ||
      ignoredWideColumns.some((alias) => header === normalize(alias))
    )
      return;
...
function findHeader(headers: string[], aliases: string[]) {
  return headers.findIndex((header) =>
    aliases.some(
      (alias) =>
        header === normalize(alias) || header.includes(normalize(alias)),
    ),
  );
}
...
function inferDateFromNearbyRows(matrix: Matrix, headerRowIndex: number) {
  const before = matrix
    .slice(Math.max(0, headerRowIndex - 4), headerRowIndex)
    .flat()
    .map(text)
    .join(" ");
...
function inferDateFromSheetName(value: string) {
  ...
  const found = months.find(([, aliases]) =>
    aliases.some((month) => new RegExp(`\\b${month}\\b`).test(normalized)),
  );
  if (!found) return "";
  const [month] = found;
...
function isoDate(value: unknown) {
  ...
  const direct = /^\d{4}-\d{2}-\d{2}$/.test(textValue)
...
function isUsableItemText(value: string, code: string) {
  ...
  if (ignoredWideColumns.some((alias) => normalized === normalize(alias)))
    return false;
